...
export function useSchedules() {
  ...
  // 用于外部更新查询条件
  function updateQueryParams(newParams: { sortField: string; sortOrder: string; filters: Record<string, string[]> }) {
  ...
  function fetchSchedules() {
    ...
    http.request('get', '/portalapi/calendar/', { params })
      .then((res: any) => {
  ...
  function handleSave() {
    ...
    http.request('post', `/portalapi/calendar/?action=${action}`, {
      data: { ...form }
    })
      .then((res: any) => {
        if (res.status === 'success') {
          ...
          fetchSchedules();
          message(`${action === 'add' ? '添加' : '保存'}成功`, { type: 'success' });
        } else {
  ...
  function handleDelete(row: any) {
    http.request('post', `/portalapi/calendar/?action=delete`, {
      data: { id: row.id }
    })
      .then((res: any) => {
        if (res.status === 'success') {
